...
# File Name: PG&E B-19S-S NEM 2 Export Compensation Rate Creator.R
# File Location: "OSESMO/Sample Input and Output Data/Export Compensation Rate Data/PG&E B-19S-S NEM 2"
# Project: OSESMO
# Description: Converts PG&E B-19S-S NEM 2.0 export compensation rate into time-series vectors for use in OSESMO's cost and constraint matrices.
...
PGE_B19SS_NEM2_ECR_Vectors.df <- data.frame(Date_Time = seq.POSIXt(Start_Date_Time, End_Date_Time, by = paste(Data_Timestep_Length, "min")))


#### Create Vectors for Energy and Demand Charges ####

# Values are for PG&E B-19 Option S, Secondary Voltage, in US $.

Nonbypassable_Charges <- 0.01453 + 0.00093 + 0.00003 + 0.00580

Summer_Peak_Rate = 0.36430 - Nonbypassable_Charges
Summer_Partial_Peak_Rate = 0.18046 - Nonbypassable_Charges
Summer_Off_Peak_Rate = 0.11999 - Nonbypassable_Charges

Winter_Peak_Rate = 0.15367 - Nonbypassable_Charges
Winter_Off_Peak_Rate = 0.11136 - Nonbypassable_Charges
Winter_Super_Off_Peak_Rate = 0.07554 - Nonbypassable_Charges
...
PGE_B19SS_NEM2_ECR_Vectors.df <- PGE_B19SS_NEM2_ECR_Vectors.df %>%
...
PGE_B19SS_Cost_Vector_Check.df <- PGE_B19SS_NEM2_ECR_Vectors.df %>%
...
rm(PGE_B19SS_Cost_Vector_Check.df)
...
# All data
write.csv(PGE_B19SS_NEM2_ECR_Vectors.df, file.path(Clean_Rate_Data_WD, paste0(Data_Timestep_Length, "-Minute Data"), "Dataframe Format", paste0(Data_Year, "_PGE_B19SS_NEM2_ECR_Dataframe.csv")), row.names = FALSE)

# Energy Rates Variable Column
write.table(PGE_B19SS_NEM2_ECR_Vectors.df$Energy_Rates, file.path(Clean_Rate_Data_WD, paste0(Data_Timestep_Length, "-Minute Data"), "Vector Format", paste0(Data_Year, "_PGE_B19SS_NEM2_ECR_Vector.csv")), 
