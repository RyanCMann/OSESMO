...
# File Name: PG&E EV2 NEM 2 Export Compensation Rate Creator.R
# File Location: "OSESMO/Sample Input and Output Data/Export Compensation Rate Data/PG&E EV2 (NEW) NEM 2"
# Project: OSESMO
# Description: Converts PG&E EV2 NEM 2.0 export compensation rate into time-series vectors for use in OSESMO's cost and constraint matrices.
...
PGE_EV2_NEM2_ECR_Vectors.df <- data.frame(Date_Time = seq.POSIXt(Start_Date_Time, End_Date_Time, by = paste(Data_Timestep_Length, "min")))


#### Create Vectors for Export Compensation Rates ####

# Values are for PG&E EV2, in US $.

Nonbypassable_Charges <- 0.01575 + 0.00093 + 0.00004 + 0.00580

Summer_Peak_Rate = 0.49616 - Nonbypassable_Charges
Summer_Partial_Peak_Rate = 0.38567 - Nonbypassable_Charges
Summer_Off_Peak_Rate = 0.18366 - Nonbypassable_Charges

Winter_Peak_Rate = 0.36905 - Nonbypassable_Charges
Winter_Partial_Peak_Rate = 0.35235 - Nonbypassable_Charges
Winter_Off_Peak_Rate = 0.18366 - Nonbypassable_Charges
...
PGE_EV2_NEM2_ECR_Vectors.df <- PGE_EV2_NEM2_ECR_Vectors.df %>%
...
PGE_EV2_Cost_Vector_Check.df <- PGE_EV2_NEM2_ECR_Vectors.df %>%
...
# All data
write.csv(PGE_EV2_NEM2_ECR_Vectors.df, file.path(Clean_Rate_Data_WD, paste0(Data_Timestep_Length, "-Minute Data"), "Dataframe Format", paste0(Data_Year, "_PGE_EV2_NEM2_ECR_Dataframe.csv")), row.names = FALSE)

# Energy Rates Variable Column
write.table(PGE_EV2_NEM2_ECR_Vectors.df$Energy_Rates, file.path(Clean_Rate_Data_WD, paste0(Data_Timestep_Length, "-Minute Data"), "Vector Format", paste0(Data_Year, "_PGE_EV2_NEM2_ECR_Vector.csv")), 
